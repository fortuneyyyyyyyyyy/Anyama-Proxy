...
function updateSubmitState() { const form = $('#registration-form'); const required = ['first_name','last_name','category','zone'].every(name => form.elements[name].value.trim()); const categoryReady = form.elements.category.value !== '__other__' || form.elements.category_custom.value.trim(); const contact = form.elements.phone.value.trim() || form.elements.whatsapp.value.trim(); const ready = required && categoryReady && contact && form.elements.consent.checked; $('.submit', form).disabled = !ready; }
...
async function submitRegistration(event) { event.preventDefault(); const form = event.currentTarget; const feedback = $('#form-feedback'); const data = Object.fromEntries(new FormData(form).entries()); if (data.category === '__other__') data.category = data.category_custom.trim(); data.consent = form.elements.consent.checked ? 'on' : ''; delete data.category_custom; feedback.className = 'form-feedback loading'; feedback.textContent = 'Enregistrement en cours…'; try { const response = await fetch(`${API_BASE}/api/artisans`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(data) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || 'Erreur'); feedback.className = 'form-feedback success'; feedback.textContent = 'Merci ! Votre profil est enregistré et visible immédiatement dans l’annuaire.'; form.reset(); $('#custom-category-wrap').hidden = true; updateSubmitState(); } catch (error) { feedback.className = 'form-feedback error'; feedback.textContent = error.message || 'Impossible d’envoyer le formulaire.'; } }
