...
function initCookieConsent() { const banner = $('#cookie-banner'); if (!banner) return; const key = 'anyama_cookie_consent_v1'; if (localStorage.getItem(key) === 'accepted') return; banner.hidden = false; $('#cookie-accept').addEventListener('click', () => { localStorage.setItem(key, 'accepted'); banner.hidden = true; }); }
