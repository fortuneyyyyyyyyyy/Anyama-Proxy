...
function openReportForm(type) { activeReportType = type; const config = REPORT_CONFIG[type]; if (!config) return; const snapshot = $('#profile-report-snapshot'); const fields = [['Nom', activeProfile.name], ['Métier', activeProfile.category], ['Quartier', activeProfile.zone], ['Téléphone', activeProfile.phone], ['WhatsApp', activeProfile.whatsapp], ['Service', activeProfile.service], ['Description', activeProfile.description]]; snapshot.innerHTML = `<p class="snapshot-title">Informations actuelles du profil</p><dl>${fields.filter(([, value]) => value).map(([label, value]) => `<div><dt>${escapeHTML(label)}</dt><dd>${escapeHTML(value)}</dd></div>`).join('')}</dl>`; const correctionFields = [['name','Nom','text'],['category','Métier','text'],['zone','Quartier','text'],['phone','Téléphone','tel'],['whatsapp','WhatsApp','tel'],['service','Service','text'],['description','Description','textarea']]; $('#profile-correction-fields').innerHTML = type === 'error' ? `<p class="snapshot-title">Proposer les informations corrigées</p><p class="correction-help">Modifiez uniquement les champs qui doivent être corrigés. L’administration validera ou refusera chaque proposition.</p><div class="correction-fields-grid">${correctionFields.map(([key,label,inputType]) => `<label>${escapeHTML(label)}<${inputType === 'textarea' ? 'textarea' : 'input'} data-correction-field="${key}" name="correction_${key}" type="${inputType === 'textarea' ? '' : inputType}" rows="3">${inputType === 'textarea' ? escapeHTML(activeProfile[key] || '') : ''}</${inputType === 'textarea' ? 'textarea' : 'input'}></label>`).join('')}</div>` : ''; $('#report-details-field').hidden = type === 'error'; $('#reporter-email-field').hidden = type === 'error'; $('#report-form').classList.toggle('correction-mode', type === 'error'); $('#report-choices').hidden = true; $('#report-form').hidden = false; $('#report-form-kicker').textContent = config.kicker; $('#report-form-title').textContent = config.title; $('#report-form-help').textContent = config.help; $('#report-reasons').innerHTML = `<option value="" disabled>Choisissez un ou plusieurs motifs</option>${config.reasons.map(reason => `<option value="${escapeHTML(reason)}"${type === 'withdraw' && reason === config.reasons[0] ? ' selected' : ''}>${escapeHTML(reason)}</option>`).join('')}`; $('#report-feedback').className = 'form-feedback'; $('#report-feedback').textContent = ''; formContactRequirements(type); lucide.createIcons(); }
