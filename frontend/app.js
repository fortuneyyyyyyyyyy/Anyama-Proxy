...
const REPORT_CONFIG = {
  error: { title: 'Signaler une erreur', kicker: 'CORRECTION D’INFORMATION', help: 'Signalez une information qui semble incorrecte. L’administration vérifiera le profil.', reasons: ['Coordonnées incorrectes', 'Mauvais métier', 'Mauvais quartier', 'Description incorrecte', 'Autre'] },
...
function openReportForm(type) { activeReportType = type; const config = REPORT_CONFIG[type]; if (!config) return; $('#report-choices').hidden = true; $('#report-form').hidden = false; $('#report-form-kicker').textContent = config.kicker; $('#report-form-title').textContent = config.title; $('#report-form-help').textContent = config.help; $('#report-reasons').innerHTML = config.reasons.map((reason, index) => `<label class="report-reason"><input type="checkbox" name="reasons" value="${escapeHTML(reason)}" ${index === 0 && type === 'withdraw' ? 'checked' : ''}><span>${escapeHTML(reason)}</span></label>`).join(''); $('#report-feedback').className = 'form-feedback'; $('#report-feedback').textContent = ''; lucide.createIcons(); }
async function submitProfileReport(event) { event.preventDefault(); if (!activeProfile || !activeReportType) return; const form = event.currentTarget; const feedback = $('#report-feedback'); const reasons = $$('input[name="reasons"]:checked', form).map(input => input.value); if (!reasons.length) { feedback.className = 'form-feedback error'; feedback.textContent = 'Sélectionnez au moins un motif.'; return; } const payload = { artisan_id: activeProfile.id, report_type: activeReportType, reasons, details: form.elements.details.value.trim(), reporter_name: form.elements.reporter_name.value.trim(), reporter_phone: form.elements.reporter_phone.value.trim(), reporter_email: form.elements.reporter_email.value.trim() }; feedback.className = 'form-feedback loading'; feedback.textContent = 'Envoi du signalement…'; try { const response = await fetch(`${API_BASE}/api/profile-reports`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(payload) }); const result = await response.json(); if (!response.ok) throw new Error(result.error || 'Erreur'); feedback.className = 'form-feedback success'; feedback.textContent = 'Votre signalement a été transmis à l’administration.'; form.reset(); window.setTimeout(closeProfileModal, 1400); } catch (error) { feedback.className = 'form-feedback error'; feedback.textContent = error.message || 'Impossible d’envoyer le signalement.'; } }
